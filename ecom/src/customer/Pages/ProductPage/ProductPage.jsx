...
export default function ProductPage() {
  ...
  const handleFilter=(value,seactionId)=>{
    ...
    let filterValue = searchParamms.getAll(seactionId)

    if(filterValue.length>0 && filterValue[0].split(",").includes(value)){
      filterValue = filterValue[0].split(",").filter((item)=>item!==value);

      if(filterValue.length===0){
        searchParamms.delete(seactionId)
      }
    }
    else{
    ...
    if(filterValue.length>0){
      searchParamms.set(seactionId,filterValue.join(","));
    }
                    ...
                    <DisclosurePanel className="pt-6">
                      <div className="space-y-2">
                       <RadioGroup
                        ...
                        {section.options.map((option, optionIdx) => ( 
                           <>
                              <FormControlLabel onChange={(e)=>handelRadioFilterChange(e,section.id)} value={option.value} control={<Radio />} label={option.label} /> 
                            </> 
                        ))}
                        </RadioGroup>
